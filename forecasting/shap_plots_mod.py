"""Added arguments to SHAP summary plot (custom xlims). Display categorical variables as a solid color to show the feature value."""
...
def summary_legacy(
    ...
    #     shap_values[shap_values < xlims[0]] = np.nan
    #     shap_values[shap_values > xlims[1]] = np.nan
    ...
    if sort:
        ...
        if multi_class:
            feature_order = np.argsort(np.sum(np.nanmean(np.abs(shap_values), axis=1), axis=0))
        else:
            feature_order = np.argsort(np.nansum(np.abs(shap_values), axis=0))
            ...
            if features is not None and colored_feature:
                ...
                if plot_categorical:
                    ...
                    if categories.size <= 6:
                        color_palette = sns.color_palette("bright", categories.size)
                        color_map = dict(zip(categories, color_palette))
                        color_map[np.nan] = "#777777"
                        print(f"Color map for feature {feature_names[i]}: {color_map}")
                        color = [color_map[v] for v in values]
                        add_categorical_legend = True
                    else:
                        color = color if colored_feature else "#777777",
                        warnings.warn("Categorical features must have less than 6 categories for adequate coloring.")
                else:
                    color = color if colored_feature else "#777777",
                pl.scatter(
                    shaps,
                ...
                if add_categorical_legend:
                    patches = [mpatches.Patch(color=color_map[v], label=v) for v in categories]
                    pl.legend(handles=patches, loc="upper left", title=feature_names[i], title_fontsize=12, fontsize=11)
    ...
    if xlims is not None:
        pl.xlim(*xlims)
        if xlims[1] - xlims[0] > 1000:
            # use exponential format for large numbers
            def format_func(value, tick_number):
                # format the value as 10^k
                if abs(value) >= 1000:
                    power = int(np.log10(abs(value)))
                    base = value / 10**power
                    if abs(base) > 1:
                        if base == int(base):
                            return r'${%.0f} \cdot 10^{%.0f}$' % (base, power)
                        else:
                            return r'${%.1f} \cdot 10^{%.0f}$' % (base, power)
                    elif base > 0:
                            return r'$10^{%.0f}$' % power
                    else:
                        return r'$-10^{%.0f}$' % power
                else:
                    if value == int(value):
                        ...
                        return value
            # power = int(np.log10(abs(xlims[1])))
            # # set the x-axis ticks to be integer multiples of 10**power
            # nticks = 5
            # closest_upper = np.ceil(xlims[1] / 10**power)
            # closest_lower = np.floor(xlims[0] / 10**power)
            # ticks = np.linspace(closest_lower, closest_upper, nticks)
            # pl.gca().set_xticks(ticks * 10**power)

            pl.gca().xaxis.set_major_formatter(ticker.FuncFormatter(format_func))
    if plot_type == "bar":
