...
try:
    opts, args = getopt.getopt(sys.argv[1:], "t:T:e:a:c:O:E:",
                               ["target=", "task=", "exclude_prediction_attrs=", "all_features=", "c=", "overwrite=", "ef_abs_diff="])
...
kwargs = dict(target=target, task=task, exclude_prediction_attrs=exclude_prediction_attrs, all_features=all_features, c=c, overwrite=overwrite, ef_abs_diff=ef_abs_diff)
print(f"kwargs:\n{kwargs}")
...
study = error_analysis.xgb_results(**input_kwargs)
